...
- **Single ingress URL**: Tests use one URL for both the browser (`baseURL`) and backend API assertions (`BACKEND_URL`). The ingress proxy routes `/api/*` to the agent-server, `/api/automation/*` to the automation backend, and `/*` to the static frontend. Port assignment is per run: the first local run may keep the historical defaults (`9999`, `18300`, `18301`, `18000`, `18001`, `3001`, `8001`), while overlapping runs atomically lease a separate high-port block. Explicit `MOCK_LLM_*_PORT` overrides are preserved and validated for cross-service collisions.
- **State isolation**: `tests/e2e/mock-llm/run-isolation.ts` creates a unique `.tmp/mock-llm-runs/<run-id>/` namespace for state, the automation DB, project-skill repos, user-skill fixtures, and folder-workspace fixtures. Only run-owned paths are pre-cleaned and removed on normal Playwright process exit; explicit state/path overrides are never deleted. npm-mode user-skill tests therefore never write to the developer's real `~/.openhands/skills`.
- **Session identity / server reuse**: A random key is generated per test run and passed to the stack via `SESSION_API_KEY` / `OH_SESSION_API_KEYS_0` / `VITE_SESSION_API_KEY`. Mock-LLM web servers set `reuseExistingServer: false`; a run must start the stack that owns its session key instead of silently attaching to another local run. The static server injects that key into `index.html` at serve time so the frontend authenticates automatically.
- **Concurrent-run artifacts**: The run that owns the historical port set keeps the existing `test-results-mock-llm/`, `playwright-report-mock-llm/`, and `.mock-llm-markers/` paths so CI/report tooling is unchanged. Overlapping runs suffix those paths with their run id; `DoneMarkerReporter` reads `MOCK_LLM_MARKER_DIR`, preventing JSON reports, videos, traces, and completion markers from overwriting each other.
...
- **State isolation**: The Docker container uses its internal state directory (no host mount needed for tests), while all host-side fixtures live under the same per-run namespace created by `run-isolation.ts`. The config also passes reserved per-run agent-server, automation, ingress, public-mode, editor, and mock-LLM ports into the host-networked container so concurrent Docker runs do not bind the same host ports.
- **Skill test volume mounts**: Tests that create files the agent-server needs to read (skill repos, user skills) require Docker volume mounts because the container has an isolated filesystem. The Docker config mounts the current run's `skill-repos/` directory → `/tmp/mock-llm-skill-repos/` and `user-skills/` → `/home/openhands/.openhands/skills/`. Env vars `MOCK_LLM_SKILL_REPOS_HOST_DIR`, `MOCK_LLM_SKILL_REPOS_CONTAINER_DIR`, and `MOCK_LLM_USER_SKILLS_HOST_DIR` tell `skill-test-helpers.ts` which paths to use for host-side file operations vs. agent-server registration.
